...
class EnemyGroup:
    ...
    def advance(self, model):
        ...
        for en in self.__expedition:
            en.move()
            if en.health <= 0:
                self.retreat(en)  # delete the object when it is without blood
                model.money += 15  # When an enemy is killed, we get 15 unit of money
            if BASE.collidepoint(en.rect.centerx, en.rect.centery):
                self.retreat(en)  # delete the object when it reach the base
                model.hp -= 1  # When an enemy reach the base, our hp drop 1 unit
