            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.start_btn.clicked(x, y):
                        ...
                        game.run()
                        run = False
                    """music on/off according to the button"""
                    if self.mute_btn.clicked(x, y):   # music off
                        pygame.mixer.music.pause()
                    if self.sound_btn.clicked(x, y):  # music on
                        pygame.mixer.music.unpause()
            """create button frame and draw"""
            for btn in self.buttons:           # go through all the buttons
                btn.create_frame(x, y)         # create the frame for the button
                btn.draw_frame(self.menu_win)  # draw the frame for the button
